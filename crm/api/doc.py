...
def get_data(
	...
	if view_type != "kanban":
		...
		or_filters = []
		if txt:
			# Search across multiple fields for CRM Lead
			if doctype == "CRM Lead":
				search_fields = ["lead_name", "first_name", "last_name", "email", "mobile_no", "phone", "organization", "name"]
			else:
				# Default search fields for other doctypes
				search_fields = ["name"]
			...
			if (column_field in filters and filters.get(column_field) != kc.get("name")) or kc.get("delete"):
				...
				if order:
					column_data = get_records_based_on_order(
						doctype, rows, column_filters, page_length, order, or_filters if or_filters else None
					)
				else:
	...
	# Build or_filters for total count if txt is provided
	or_filters_for_count = []
	if txt:
		if doctype == "CRM Lead":
			search_fields = ["lead_name", "first_name", "last_name", "email", "mobile_no", "phone", "organization", "name"]
		else:
		...
		for field in search_fields:
			or_filters_for_count.append([doctype, field, "like", f"%{txt}%"])
	...
	return {
		...
		"total_count": frappe.get_list(doctype, filters=filters, or_filters=or_filters_for_count if or_filters_for_count else None, fields=[COUNT_NAME])[0].total_count,
		"row_count": len(data),
...
def get_records_based_on_order(doctype, rows, filters, page_length, order, or_filters=None):
	records = []
